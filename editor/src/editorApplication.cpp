...
auto EditorApplication::create_input_actions() noexcept -> void {
	...
	auto initiated_in_scene_view = InputActionConstraint{
	...
	auto translate_mode = InputAction{
		{InputType::KEYBOARD, ActionType::PRESS, ImGuiKey_W},
		[this](InputEvent const& event) {
			m_control_state.gizmo_state.op = ImGuizmo::OPERATION::TRANSLATE;
		}
	};
	auto rotate_mode = InputAction{
		{InputType::KEYBOARD, ActionType::PRESS, ImGuiKey_E},
		[this](InputEvent const& event) {
			m_control_state.gizmo_state.op = ImGuizmo::OPERATION::ROTATE;
		}
	};
	auto scale_mode = InputAction{
		{InputType::KEYBOARD, ActionType::PRESS, ImGuiKey_R},
		[this](InputEvent const& event) {
			m_control_state.gizmo_state.op = ImGuizmo::OPERATION::SCALE;
		}
	};
	auto toggle_snap = InputAction{
		{InputType::KEYBOARD, ActionType::PRESS, ImGuiKey_X},
		[this](InputEvent const& event) {
			m_control_state.gizmo_state.snap = !m_control_state.gizmo_state.snap;
		}
	};

	auto delete_obj = InputAction{
		{InputType::KEYBOARD, ActionType::PRESS, ImGuiKey_Delete},
		[this](InputEvent const& event) {
	...
	auto focus_on_obj = InputAction{
		...
		[this](InputEvent const& event) {
			...
			LookAtParams params;
			params.center = m_control_state.get_cur_obj()->get_transform(TransformSpace::WORLD).get_position();
			params.eye = params.center + local_bound.get_extent() * 2.0f;
			params.up = glm::vec3{0, 1, 0};
			m_cam.set(params);
		}
	};
	auto select_obj = InputAction{
		{InputType::MOUSE, ActionType::PRESS, ImGuiMouseButton_Left},
		[this](InputEvent const& event) {
	...
	auto deselect_obj = InputAction{
		{InputType::KEYBOARD, ActionType::PRESS, ImGuiKey_Escape},
		[this](InputEvent const& event) {
	...
	auto cam_pan = InputAction{
		{InputType::MOUSE, ActionType::HOLD, ImGuiMouseButton_Left},
		[this, get_cam_accel_factor](InputEvent const& event) {
			auto dolly = glm::vec3{
	...
	auto cam_rotate = InputAction{
		{InputType::MOUSE, ActionType::HOLD, ImGuiMouseButton_Right},
		[this](InputEvent const& event) {
	...
	auto cam_zoom = InputAction{
		{InputType::MOUSE, ActionType::SCROLL, ImGuiMouseButton_Middle},
		[this, get_cam_accel_factor](InputEvent const& event) {
...
auto EditorApplication::draw_scene_panel() noexcept -> void {
	...
	if (ImGui::CollapsingHeader("Performance", ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		{
			auto cap = m_control_state.unlimited_fps
...
auto EditorApplication::draw_scene_viewport(TextureHandle render_buf) noexcept -> void {
	...
	if (get_cur_renderer().valid()) {
		...
		if (m_control_state.get_cur_obj()) {
			...
			if (Manipulate(
				value_ptr(m_cam.get_view()),
				value_ptr(m_cam.get_projection()),
				gizmo_state.op,
				gizmo_state.mode,
				value_ptr(mat),
				nullptr,
