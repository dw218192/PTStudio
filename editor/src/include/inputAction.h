...
namespace PTS {
	...
	struct Input {
		...
		auto operator==(Input const& other) const noexcept -> bool {
			return input_type == other.input_type && action_type == other.action_type && key_or_button == other.
				key_or_button;
		}
		auto operator!=(Input const& other) const noexcept -> bool {
			return !(*this == other);
	...
	struct InputEvent {
		InputEvent(Input input, glm::vec2 mouse_pos, glm::ivec2 screen_size,
		           ...
		           float time) noexcept
			: input{std::move(input)}, last_mouse_pos{mouse_pos}, mouse_pos{mouse_pos},
			  normalized_mouse_pos{mouse_pos / glm::vec2(screen_size)},
			  mouse_scroll_delta{mouse_scroll_delta}, initiated_window{initiated_window}, time{
				  time
			  } {}

		InputEvent(Input input, glm::vec2 mouse_pos, glm::vec2 last_mouse_pos, glm::ivec2 screen_size,
		           glm::vec2 mouse_scroll_delta,
		           std::string_view initiated_window, float time) noexcept
