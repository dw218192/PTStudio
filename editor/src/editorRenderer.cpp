...
auto EditorRenderer::draw_outline(View<Camera> cam_view,
	...
	TL_CHECK_AND_PASS(m_outline.render_buf->bind());
	{
		...
		TL_CHECK_AND_PASS(m_outline.shaders[0]->bind());
		{
			...
			TL_CHECK_AND_PASS(
				m_outline.shaders[0]->set_uniform(k_uniform_model, obj.get().get_transform(TransformSpace::WORLD).
					get_matrix()));
		...
		{
			TL_CHECK_AND_PASS(
				m_outline.shaders[1]->set_texture(k_uniform_screen_texture, m_outline.render_buf->get_texture(
			...
			TL_CHECK_AND_PASS(
				m_outline.shaders[1]->set_uniform(k_uniform_texel_size, 1.0f / glm::vec2(m_config.width, m_config.height
				)));
	...
	TL_CHECK_AND_PASS(m_outline.shaders[2]->bind());
	{
		...
		{
			TL_CHECK_AND_PASS(
				m_outline.shaders[2]->set_texture(k_uniform_screen_texture, m_outline.render_buf->get_texture(
...
auto EditorRenderer::render(View<Camera> cam_view) noexcept -> tl::expected<TextureHandle, std::string> {
	...
	if (!m_scene->get_objects_of_type<Light>().empty()) {
		...
		TL_CHECK(m_light_gizmo_data.shader->bind());
		{
			...
			TL_CHECK(
				m_light_gizmo_data.shader->set_texture(k_uniform_sprite_texture, m_light_gizmo_data.texture.get(), 0));
			...
			for (auto&& light : m_scene->get_objects_of_type<Light>()) {
				TL_CHECK(
					m_light_gizmo_data.shader->set_uniform(k_uniform_sprite_world_pos, light.get_transform(
						TransformSpace::WORLD).get_position()));
				TL_CHECK(m_light_gizmo_data.shader->set_uniform(k_uniform_sprite_tint, light.get_color()));
...
auto EditorRenderer::try_get_obj_data(
	View<RenderableObject> obj) noexcept -> tl::expected<Ref<PerObjectData>, std::string> {
