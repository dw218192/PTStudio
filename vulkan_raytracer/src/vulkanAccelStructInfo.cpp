...
[[nodiscard]] auto PTS::Vk::VulkanAccelStructInfo::create(
    ...
    auto build_sizes = dev->getAccelerationStructureBuildSizesKHR(
        vk::AccelerationStructureBuildTypeKHR::eDevice, geom_build_info, max_primitive_count);
    auto accel_buf = VulkanBufferInfo{};
    TL_TRY_ASSIGN(accel_buf, VulkanBufferInfo::create(dev, VulkanBufferInfo::Type::AccelStorage,
    ...
    TL_TRY_ASSIGN(scratch_buf, VulkanBufferInfo::create(dev, VulkanBufferInfo::Type::Scratch,
                                                        build_sizes.buildScratchSize));

    geom_build_info.setScratchData(scratch_buf.get_device_addr())
    ...
    return VulkanAccelStructInfo{{std::move(accel)},
                                 std::move(accel_buf),
                                 std::move(scratch_buf),
                                 geom_build_info.flags,
                                 std::vector<vk::AccelerationStructureGeometryKHR>{
                                     geom_build_info.pGeometries,
                                     geom_build_info.pGeometries + geom_build_info.geometryCount}};
}

[[nodiscard]] auto PTS::Vk::VulkanBottomAccelStructInfo::create(
    VulkanDeviceInfo const& dev, VulkanCmdPoolInfo const& cmd_pool,
    RenderableObject const& obj) -> tl::expected<VulkanBottomAccelStructInfo, std::string> {
    ...
    std::transform(
        obj.get_vertices().begin(), obj.get_vertices().end(), std::back_inserter(vertices),
        [](auto const& vert) { return PTS::Vk::VulkanRayTracingShaders::VertexData{vert}; });
    TL_TRY_ASSIGN(vert_buf,
    ...
    TL_TRY_ASSIGN(accel, PTS::Vk::VulkanAccelStructInfo::create(dev, cmd_pool, build_info, prim_cnt,
                                                                prim_cnt));
...
[[nodiscard]] auto PTS::Vk::VulkanTopAccelStructInfo::create(VulkanDeviceInfo const& dev,
    ...
    TL_TRY_ASSIGN(accel_ins_buf, PTS::Vk::VulkanBufferInfo::create(
                                     dev, PTS::Vk::VulkanBufferInfo::Type::AccelInput,
    ...
    auto accel = PTS::Vk::VulkanAccelStructInfo{};
    TL_TRY_ASSIGN(accel, PTS::Vk::VulkanAccelStructInfo::create(dev, cmd_pool, build_info, ins_cnt,
                                                                PTS::k_max_objs));
...
[[nodiscard]] auto PTS::Vk::VulkanTopAccelStructInfo::add_instance(
    ...
    glm::mat4 const& transform) noexcept -> tl::expected<size_t, std::string> {
    ...
    TL_CHECK(update_accel_gpu(
        ...
        vk::BuildAccelerationStructureModeKHR::eBuild, 0, m_instances.size() - 1));
    return idx;
}
...
[[nodiscard]] auto PTS::Vk::VulkanTopAccelStructInfo::to_mat4x3(glm::mat4 const& mat) noexcept
    ...
    return vk::TransformMatrixKHR{
        std::array{std::array{mat[0][0], mat[1][0], mat[2][0], mat[3][0]},
...
[[nodiscard]] auto PTS::Vk::VulkanTopAccelStructInfo::update_accel_gpu(
    vk::BuildAccelerationStructureModeKHR build_type, size_t from,
    ...
    auto build_range_info = vk::AccelerationStructureBuildRangeInfoKHR{}
                                ...
                                .setPrimitiveOffset(static_cast<uint32_t>(
                                    from * sizeof(vk::AccelerationStructureInstanceKHR)))
                                .setTransformOffset(0);
    if (m_accel.geometries[0].geometry.instances.data.deviceAddress !=
        m_ins_mem.get_device_addr()) {
