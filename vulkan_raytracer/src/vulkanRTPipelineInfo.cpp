...
struct EmbeddedFsIncluder : shaderc::CompileOptions::IncluderInterface {
    ...
    auto GetInclude(char const* requested_source, shaderc_include_type type,
        ...
        if (type == shaderc_include_type_standard) {
            inc_src_res =
                PTS::try_get_embedded_res(cmrc::core_resources::get_filesystem(), file_path);
        } else {
            inc_src_res = PTS::try_get_embedded_res(
                cmrc::vulkan_raytracer_resources::get_filesystem(), file_path);
        }
...
[[nodiscard]] auto create_shader_glsl(PTS::Vk::VulkanDeviceInfo const& dev, std::string const& src,
    ...
    try {
        ...
        auto const prep_res =
            compiler.PreprocessGlsl(src, to_shaderc_stage(stage), name.data(), options);
        ...
        auto const res =
            compiler.CompileGlslToSpv(prepped_src, to_shaderc_stage(stage), name.data(), options);
        if (res.GetCompilationStatus() != shaderc_compilation_status_success) {
            return TL_ERROR("shader compilation failed:\n{}\n{}", res.GetErrorMessage(),
                            prepped_src);
        }
        auto const sprv_code = std::vector<uint32_t>{res.cbegin(), res.cend()};
        auto shader = dev->createShaderModuleUnique(vk::ShaderModuleCreateInfo{
...
auto PTS::Vk::VulkanRTPipelineInfo::create(VulkanDeviceInfo const& dev,
    ...
    TL_TRY_ASSIGN(shader_infos[0], create_shader_glsl(dev, rgen_shader_src, "rgen_shader",
                                                      vk::ShaderStageFlagBits::eRaygenKHR));
    TL_TRY_ASSIGN(shader_infos[1], create_shader_glsl(dev, miss_shader_src, "miss_shader",
                                                      vk::ShaderStageFlagBits::eMissKHR));
    TL_TRY_ASSIGN(shader_infos[2], create_shader_glsl(dev, chit_shader_src, "chit_shader",
                                                      vk::ShaderStageFlagBits::eClosestHitKHR));

    auto shader_stages = std::array{
        vk::PipelineShaderStageCreateInfo{vk::PipelineShaderStageCreateFlags{},
                                          vk::ShaderStageFlagBits::eRaygenKHR, *shader_infos[0],
                                          "main", nullptr},
        vk::PipelineShaderStageCreateInfo{vk::PipelineShaderStageCreateFlags{},
                                          vk::ShaderStageFlagBits::eMissKHR, *shader_infos[1],
                                          "main", nullptr},
        vk::PipelineShaderStageCreateInfo{vk::PipelineShaderStageCreateFlags{},
                                          vk::ShaderStageFlagBits::eClosestHitKHR, *shader_infos[2],
                                          "main", nullptr},
    };
    auto shader_groups =
        std::array{// group0  raygen
    ...
    try {
        ...
        auto min_alignment =
            dev.physical_device.getProperties().limits.minStorageBufferOffsetAlignment;
        ...
        auto desc_sets = std::array<VulkanDescSetInfo,
        ...
        desc_sets[cur_set++]
            .add_binding(
                vk::DescriptorSetLayoutBinding{}
                    .setBinding(
                        VulkanRayTracingShaders::RayTracingBindings::ACCEL_STRUCT_BINDING.binding)
                    .setDescriptorType(vk::DescriptorType::eAccelerationStructureKHR)
                    .setDescriptorCount(1)
                    .setStageFlags(vk::ShaderStageFlagBits::eRaygenKHR |
                                   vk::ShaderStageFlagBits::eClosestHitKHR),
                vk::DescriptorBindingFlags{}, vk::WriteDescriptorSet{}.setPNext(&accel_info))
            .add_binding(
                vk::DescriptorSetLayoutBinding{}
                    .setBinding(
                        VulkanRayTracingShaders::RayTracingBindings::OUTPUT_IMAGE_BINDING.binding)
                    .setDescriptorType(vk::DescriptorType::eStorageImage)
                    .setDescriptorCount(1)
                    .setStageFlags(vk::ShaderStageFlagBits::eRaygenKHR),
                vk::DescriptorBindingFlags{}, vk::WriteDescriptorSet{}.setImageInfo(img_info))
            .add_binding(
                vk::DescriptorSetLayoutBinding{}
                    .setBinding(
                        VulkanRayTracingShaders::RayTracingBindings::MATERIALS_BINDING.binding)
                    .setDescriptorType(vk::DescriptorType::eUniformBuffer)
                    .setDescriptorCount(1)
                    .setStageFlags(vk::ShaderStageFlagBits::eClosestHitKHR),
                vk::DescriptorBindingFlags{}, vk::WriteDescriptorSet{}.setBufferInfo(mat_buf_info))
            .add_binding(
                vk::DescriptorSetLayoutBinding{}
        ...
        desc_sets[cur_set++].add_binding(
            ...
            vk::WriteDescriptorSet{}.setBufferInfo(null_buf_infos));
        ...
        auto desc_set_layouts =
            std::array<vk::DescriptorSetLayout,
                       VulkanRayTracingShaders::RayTracingBindings::k_num_sets>{};
        std::transform(desc_sets.begin(), desc_sets.end(), desc_set_layouts.begin(),
                       [](VulkanDescSetInfo const& info) { return info.get_layout(); });
        ...
        auto pipeline = dev->createRayTracingPipelineKHRUnique(
            ...
            vk::RayTracingPipelineCreateInfoKHR{}
                ...
                .setMaxPipelineRayRecursionDepth(
                    1)  // for possible visibility tests in the chit shader
        ...
        auto props =
            dev.physical_device.getProperties2<vk::PhysicalDeviceProperties2,
        ...
        if (dev->getRayTracingShaderGroupHandlesKHR(*pipeline.value, 0, group_count, sbt_size,
        ...
        TL_TRY_ASSIGN(raygen_buf,
                      VulkanBufferInfo::create(dev, VulkanBufferInfo::Type::ShaderBindingTable,
                                               handle_size, tcb::span{handles.data(), 1}));
        auto miss_buf = VulkanBufferInfo{};
        TL_TRY_ASSIGN(miss_buf, VulkanBufferInfo::create(
                                    dev, VulkanBufferInfo::Type::ShaderBindingTable, handle_size,
        ...
        return VulkanRTPipelineInfo{{std::move(pipeline.value)},
...
auto PTS::Vk::VulkanRTPipelineInfo::bind_indices(VulkanDeviceInfo const& dev, int index,
    ...
    TL_TRY_ASSIGN(index_buf, VulkanBufferInfo::create(
                                 dev, VulkanBufferInfo::Type::Storage,
...
auto PTS::Vk::VulkanRTPipelineInfo::bind_vertex_attribs(VulkanDeviceInfo const& dev, int index,
    ...
    std::transform(vertices.begin(), vertices.end(), std::back_inserter(vertex_attribs),
                   [](Vertex const& v) { return VulkanRayTracingShaders::VertexAttribData{v}; });

    TL_TRY_ASSIGN(vertex_buf,
                  VulkanBufferInfo::create(
                      dev, VulkanBufferInfo::Type::Storage,
    ...
    auto write_set =
        vk::WriteDescriptorSet{}
            .setDstSet(
                *desc_sets[VulkanRayTracingShaders::RayTracingBindings::VERTEX_ATTRIBS_BINDING.set])
            .setDstBinding(
                VulkanRayTracingShaders::RayTracingBindings::VERTEX_ATTRIBS_BINDING.binding)
            .setDescriptorCount(1)
            .setDstArrayElement(index)
