...
struct VulkanRTPipelineInfo : VulkanInfo<vk::UniquePipeline> {
    ...
    */
    [[nodiscard]] auto bind_indices(VulkanDeviceInfo const& dev, int index, tcb::span<unsigned const> indices) noexcept
     ...
     */
    [[nodiscard]] auto bind_vertex_attribs(VulkanDeviceInfo const& dev, int index, tcb::span<Vertex const> vertices) noexcept
        -> tl::expected<void, std::string>;

    [[nodiscard]] auto get_desc_sets() {
        auto ret = std::array<vk::DescriptorSet, RayTracingBindings::k_num_sets>{};
        std::transform(
            std::begin(desc_sets),
            std::end(desc_sets),
            std::begin(ret),
    ...
    // Desc Set related
    std::array<VulkanDescSetInfo, RayTracingBindings::k_num_sets> desc_sets{};
