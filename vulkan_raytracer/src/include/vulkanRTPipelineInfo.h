...
struct VulkanRTPipelineInfo : VulkanInfo<vk::UniquePipeline> {
    ...
    [[nodiscard]] auto get_desc_sets() {
        auto ret = std::array<vk::DescriptorSet,
                              VulkanRayTracingShaders::RayTracingBindings::k_num_sets>{};
        std::transform(std::begin(desc_sets), std::end(desc_sets), std::begin(ret),
                       [](auto const& desc_set) { return *desc_set; });
        return ret;
    }
    ...
    // Desc Set related
    std::array<VulkanDescSetInfo, VulkanRayTracingShaders::RayTracingBindings::k_num_sets>
        desc_sets{};
