...
auto GLTexture::create_tex(unsigned width, unsigned height, GLenum format,
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, format,  // RGB color format
...
auto GLTexture::create(std::string_view img_file, FileFormat type,
                       std::initializer_list<GLParam> params) noexcept
    -> tl::expected<GLTextureRef, std::string> {
...
auto GLTexture::create(unsigned width, unsigned height, GLenum format, GLuint handle) noexcept
    ...
    auto ret =
        GLTextureRef{new GLTexture{width, height, channels, format, handle}, GLResourceDeleter{}};
    return ret;
}

GLTexture::GLTexture(unsigned width, unsigned height, unsigned channels, GLenum format,
                     GLuint handle)
