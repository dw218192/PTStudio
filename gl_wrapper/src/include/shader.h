...
namespace PTS {
	...
	struct Shader final : GLResource {
		...
		[[nodiscard]] static auto from_file(ShaderType type,
		                                    std::string_view file) noexcept -> tl::expected<ShaderRef, std::string>;
		[[nodiscard]] static auto from_src(ShaderType type,
	...
	struct ShaderProgram final : GLResource {
		...
		*/
		template <typename UniformType, typename = std::enable_if_t<!std::is_same_v<
			          std::decay_t<UniformType>, UniformVar>>>
