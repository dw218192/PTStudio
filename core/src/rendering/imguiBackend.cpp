...
namespace {
GLFWwindow* get_glfw_window(const IWindowing& windowing) {
    auto handle = windowing.native_handle();
    if (handle.type != WindowingType::glfw || !handle.window_handle) {
        throw std::runtime_error("ImGui backend requires a GLFW window handle");
    }
    return static_cast<GLFWwindow*>(handle.window_handle);
}
}  // namespace
...
struct ImGuiBackend::Impl {
    ...
    virtual auto register_texture(vk::Sampler sampler, vk::ImageView view, vk::ImageLayout layout)
        -> ImTextureID = 0;
    virtual void unregister_texture(ImTextureID id) = 0;
};

class VulkanGlfwImGuiBackend final : public ImGuiBackend::Impl {
   public:
    VulkanGlfwImGuiBackend(GLFWwindow* window, VulkanPresent& present, VulkanRhi& rhi,
                           LoggingManager& logging_manager)
        : m_window(window), m_present(present), m_rhi(rhi) {
        m_logger = logging_manager.get_logger_shared("ImGuiBackend");
        ...
        create_sync_objects();
        init_imgui_backend();
        m_logger->info("ImGui backend initialized");
    }

    ~VulkanGlfwImGuiBackend() override {
        if (m_initialized) {
        ...
        cleanup_swapchain();
        m_logger->info("ImGui backend destroyed");
    }
    ...
    void render(bool framebuffer_resized) override {
        ...
        (void) m_rhi.device().waitForFences(m_in_flight_fences[frame_index].get(), true,
                                            UINT64_MAX);

        uint32_t image_index = 0;
        auto acquire_result = m_present.acquire_next_image(
    ...
    auto register_texture(vk::Sampler sampler, vk::ImageView view, vk::ImageLayout layout)
    ...
    void unregister_texture(ImTextureID id) override {
        if (id) {
   ...
   private:
    ...
    void create_command_buffers() {
        static constexpr uint32_t k_frames_in_flight = 2;
        m_command_buffers =
            m_rhi.device().allocateCommandBuffersUnique(vk::CommandBufferAllocateInfo{
...
ImGuiBackend::ImGuiBackend(IWindowing& windowing, IPresent& present, VulkanRhi& rhi,
                           LoggingManager& logging_manager) {
    auto* vulkan_present = dynamic_cast<VulkanPresent*>(&present);
    if (!vulkan_present) {
        throw std::runtime_error("ImGui backend requires Vulkan present implementation");
    }
    auto* window = get_glfw_window(windowing);
    m_impl =
        std::make_unique<VulkanGlfwImGuiBackend>(window, *vulkan_present, rhi, logging_manager);
}
...
void ImGuiBackend::render(bool framebuffer_resized) {
    m_impl->render(framebuffer_resized);
}

void ImGuiBackend::resize() {
    m_impl->resize();
}

auto ImGuiBackend::register_texture(vk::Sampler sampler, vk::ImageView view, vk::ImageLayout layout)
    -> ImTextureID {
    return m_impl->register_texture(sampler, view, layout);
}

void ImGuiBackend::unregister_texture(ImTextureID id) {
    m_impl->unregister_texture(id);
}
}  // namespace pts::rendering
