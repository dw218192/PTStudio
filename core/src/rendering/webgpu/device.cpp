...
Device::Device(WGPUInstance instance, WGPUDevice device, WGPUQueue queue,
    ...
    // Enforce invariants: all handles must be non-null
    if (m_instance == nullptr) {
        throw std::runtime_error("WebGPU Device: instance handle is null");
    }
    if (m_device == nullptr) {
        throw std::runtime_error("WebGPU Device: device handle is null");
    }
    if (m_queue == nullptr) {
        throw std::runtime_error("WebGPU Device: queue handle is null");
    }
    if (!m_logger) {
        throw std::runtime_error("WebGPU Device: logger is null");
    }
...
auto Device::create(std::shared_ptr<spdlog::logger> logger) -> Device {
    if (!logger) {
        throw std::runtime_error("WebGPU Device::create: logger is null");
    }
    ...
    device_lost_callback.callback = [](WGPUDevice const*, WGPUDeviceLostReason reason,
        ...
        log->error("[WebGPU Device Lost] Reason: {}, Message: {}", reason_str,
