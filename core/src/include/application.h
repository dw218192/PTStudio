...
namespace PTS {
DECL_ENUM(LogLevel, Debug, Info, Warning, Error, StdOut, StdErr);
...
struct Application {
    ...
    auto set_max_log_cnt(unsigned cnt) {
        m_max_line_cnt = cnt;
    }
    /**
   ...
   protected:
    struct LogDesc {
        ...
        LogDesc(LogLevel level, std::string msg) : level{level}, msg{std::move(msg)} {
        }
    };

    virtual void on_log_added() = 0;
    NODISCARD auto get_logs() const -> View<std::deque<LogDesc>> {
        return m_logs;
    }
    auto clear_logs() {
        m_logs.clear();
