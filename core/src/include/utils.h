...
#define TL_CHECK(func_call)                     \
    do {                                        \
        auto res = func_call;                   \
        if (!res) return TL_ERROR(res.error()); \
    } while (0)
#define TL_CHECK_AND_PASS(func_call) \
    do {                             \
        auto res = func_call;        \
        if (!res) return res;        \
    } while (0)
#define TL_TRY_ASSIGN(out, func_call)           \
    do {                                        \
        auto res = func_call;                   \
        if (!res) return TL_ERROR(res.error()); \
        out = std::move(res.value());           \
    } while (0)
#define TL_TRY_ASSIGN_AND_PASS(out, func_call) \
    do {                                       \
        auto res = func_call;                  \
        if (!res) return res;                  \
        out = std::move(res.value());          \
...
// non-fatal errors
#define CHECK_GL_ERROR_NON_FATAL(app, level)                               \
    do {                                                                   \
        auto err = glGetError();                                           \
        if (err != GL_NO_ERROR) (app).log(level, glewGetErrorString(err)); \
    } while (0)

#define TL_CHECK_NON_FATAL(app, level, func_call) \
    do {                                          \
        auto res = func_call;                     \
        if (!res) (app)->log(level, res.error()); \
    } while (0)

#define TL_TRY_ASSIGN_NON_FATAL(out, app, level, func_call) \
    do {                                                    \
        auto res = func_call;                               \
        if (!res)                                           \
            (app)->log(level, res.error());                 \
        else {                                              \
            out = std::move(res.value());                   \
        }                                                   \
    } while (0)
...
#define DECL_ENUM(name, ...) enum class name { __VA_ARGS__, __COUNT }

/**
 * @brief Declares a static init method that will be called the first time the class is
 * instantiated.
