...
struct ModifierPack {
    ...
    constexpr ModifierPack(Modifiers... args)
...
struct TField {
    ...
    void on_change(FieldType const& old_val,
                   FieldType const& new_val,
                   ClassType& obj) const {
        for (auto& callback : runtime_info.on_change_callbacks) {
            callback(*this, old_val, new_val, obj);
        }
    }
    void unregister_on_change_callback(int idx) const {
    ...
    void unregister_all_on_change_callbacks() const {
...
struct TFieldRuntime {
    ...
    using callback_type = std::function<void(
        TField<ClassType, FieldType, ModifierPackType> const&,
        FieldType const&,
        FieldType const&,
        ClassType&)>;
...
struct has_tfield_interface<
    ...
    : std::enable_if_t<
          std::conjunction_v<
              std::is_same<
                  FieldType,
                  typename T<ClassType, FieldType, ExtraTypes...>::type>,
              std::is_same<
                  FieldType,
                  std::decay_t<
...
struct FieldInfo {
    ...
    auto operator==(FieldInfo const& other) const noexcept -> bool {
        return var_name == other.var_name && type_name == other.type_name &&
               index == other.index;
    }
    auto operator!=(FieldInfo const& other) const noexcept -> bool {
        return !(*this == other);
    ...
    template <typename TemplatedFieldInfo,
              typename Reflected,
              typename = std::enable_if_t<
                  Traits::has_tfield_interface_v<TemplatedFieldInfo>>>
...
#define BEGIN_REFLECT_IMPL(_cls, _counter, _parent)                      \
    ...
    struct _for_each_field_impl<-1, F> {                                 \
        static constexpr auto do_it(F&& func) {                          \
            if constexpr (Traits::is_reflectable_v<_parent_type>) {      \
                Traits::get_for_each_field_impl<_parent_type, F>::do_it( \
                    std::forward<F>(func));                              \
            }                                                            \
        }                                                                \
    }
...
#define FIELD_IMPL(_counter, _var_type, _var_name, _init, ...)                 \
    ...
    static constexpr auto _##_var_name##_modifiers =                           \
        ModifierPack{__VA_ARGS__};                                             \
    static inline auto _##_var_name##_runtime_field_info =                     \
        TFieldRuntime<_my_type, _var_type,                                     \
    ...
    static constexpr auto _##_var_name##_field_info =                          \
        TField<_my_type, _var_type,                                            \
               decltype(_my_type::_##_var_name##_modifiers)>{                  \
            _counter - _init_count - 1, #_var_name, #_var_type,                \
            _##_var_name##_runtime_field_info, _##_var_name##_modifiers};      \
    friend struct TFieldRuntime<_my_type, _var_type,                           \
                                decltype(_my_type::_##_var_name##_modifiers)>; \
    friend struct TField<_my_type, _var_type,                                  \
                         decltype(_my_type::_##_var_name##_modifiers)>;        \
    template <typename F>                                                      \
    struct _for_each_field_impl<_counter - _init_count - 1, F> {               \
        static constexpr auto do_it(F&& func) {                                \
            _for_each_field_impl<_counter - _init_count - 2, F>::do_it(        \
                std::forward<F>(func));                                        \
...
#define END_REFLECT_IMPL(_counter)                                           \
   ...
   public:                                                                   \
    ...
    static constexpr auto total_field_count() {                              \
        return field_count() + Traits::get_num_members<_parent_type>::value; \
    }                                                                        \
    template <typename F>                                                    \
...
#define DECL_DYNAMIC_INFO()                                                   \
   public:                                                                    \
    ...
    template <typename T>                                                     \
    std::enable_if_t<Traits::has_dynamic_info<T>::value &&                    \
                         std::is_base_of_v<_my_type, T>,                      \
                     T*>                                                      \
    as() {                                                                    \
...
struct hash<PTS::FieldInfo> {
    auto operator()(PTS::FieldInfo const& info) const noexcept -> size_t {
        return hash<std::string>{}(std::string{info.type_name} + " " +
                                   std::string{info.var_name} + " " +
                                   std::to_string(info.index));
    }
};
template <>
struct hash<PTS::TypeErasedIterator> {
    auto operator()(PTS::TypeErasedIterator const& it) const noexcept
        -> size_t {
