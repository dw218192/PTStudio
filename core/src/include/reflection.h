...
#pragma region DynamicTypeInfo
	...
	struct Type {
		std::size_t normalized_type_hash{
			0
		}; // no good way to get a readable name as of C++17
		...
		static auto of() -> Type {
			...
			if constexpr (std::is_array_v<raw_t>) {
				...
				ret.m_contained_type = std::make_shared<Type>(
					of<std::remove_pointer_t<std::decay_t<raw_t>>>());
			}
...
#define FIELD_IMPL(_counter, _var_type, _var_name, _init, ...)               \
    ...
    static constexpr auto _##_var_name##_modifiers =                         \
        PTS::ModifierPack{__VA_ARGS__};                                      \
    static inline auto _##_var_name##_runtime_field_info =                   \
        PTS::TFieldRuntime<_my_type, _var_type,                              \
    ...
    static constexpr auto _##_var_name##_field_info =                        \
        PTS::TField<_my_type, _var_type,                                     \
                    decltype(_my_type::_##_var_name##_modifiers)>{           \
            _counter - _init_count - 1, #_var_name, #_var_type,              \
            _##_var_name##_runtime_field_info, _##_var_name##_modifiers};    \
    friend struct PTS::TFieldRuntime<                                        \
        _my_type, _var_type, decltype(_my_type::_##_var_name##_modifiers)>;  \
    friend struct PTS::TField<_my_type, _var_type,                           \
                              decltype(_my_type::_##_var_name##_modifiers)>; \
    template <typename F>                                                    \
...
#define DECL_DYNAMIC_INFO()                                                   \
   public:                                                                    \
    ...
    template <typename T>                                                     \
    std::enable_if_t<PTS::Traits::has_dynamic_info<T>::value &&               \
