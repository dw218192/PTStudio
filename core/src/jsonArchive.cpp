...
struct is_pointer_or_pointer_container {
    ...
    template <typename U>
    static constexpr auto test(int) -> std::enable_if_t<std::is_pointer_v<U>, int> {
        return POINTER;
    }
...
gather_objects(ObjectOrDerived& obj) {
    ...
    ObjectOrDerived::for_each_field([&obj](auto field) {
        ...
        if constexpr (PTS::Traits::is_container<type>::value) {
            ...
            if constexpr (PTS::Traits::is_reflectable<value_type>::value &&
                          std::is_base_of_v<PTS::Object, value_type>) {
                for (auto& elem : field.get(obj)) {
                    gather_objects(elem);
                }
            }
        } else if constexpr (PTS::Traits::is_reflectable<type>::value &&
                             std::is_base_of_v<PTS::Object, type>) {
...
std::enable_if_t<PTS::Traits::is_reflectable<ObjectOrDerived>::value &&
                 std::is_base_of_v<PTS::Object, ObjectOrDerived>>
from_json_handle_pointers(nlohmann::json const& json, ObjectOrDerived& obj,
                          TemplatedFieldInfo const& info) {
...
namespace PTS {
template <typename T>
auto to_json(nlohmann::json& json, T const& tp) -> std::enable_if_t<Traits::is_tuple_like_v<T>> {
    std::apply([&json](auto const&... args) { (json.push_back(args), ...); }, tp);
}
...
template <typename ObjectOrDerived>
std::enable_if_t<std::is_base_of_v<Object, ObjectOrDerived>> to_json(nlohmann::json& json,
                                                                     ViewPtr<ObjectOrDerived> ptr) {
    if (!ptr) {
...
template <typename ObjectOrDerived>
std::enable_if_t<std::is_base_of_v<Object, ObjectOrDerived>> to_json(
    nlohmann::json& json, ObserverPtr<ObjectOrDerived> ptr) {
...
std::enable_if_t<Traits::is_reflectable<Reflected>::value> to_json(nlohmann::json& json,
...
// 2. if a new field is present in the reflected type but not in the json, it will be initialized
// with the default value
template <typename Reflected>
std::enable_if_t<Traits::is_reflectable<Reflected>::value> from_json(nlohmann::json const& json,
                                                                     Reflected& reflected) {
    Reflected::for_each_field([&reflected, &json](auto field) {
        if constexpr (field.template has_modifier<MSerialize>()) {
        ...
        for (auto const kvp : g_id_to_object) {
            ...
            obj.dyn_for_each_field([&error](FieldInfo const& field) {
                if (g_pointer_to_id.count(field)) {
                    auto const type =
                        field.type.is_container ? field.type.contained_type() : field.type;
                    // ensure that the field is a pointer
                    if (!type.is_pointer) {
                        error += fmt::format(FMT_STRING("Field {} of type {}: is not a pointer but "
                                                        "is in g_pointer_to_id\n"),
                                             field.var_name, field.type.to_string());
                        return;
                        // ensure that the field is a pointer to Object or derived type
                    } else if (type != Type::of<Object*>() &&
                               !type.pointed_to_type().has_common_base_with(Type::of<Object>())) {
                        error += fmt::format(
                            FMT_STRING(
                                "Field {} of type {}: non-Object pointers are not supported\n"),
                            field.var_name, field.type.to_string());
                        return;
                    }

                    // if the field is a pointer container or a pointer, link it to the proper
                    // object
                    ...
                    for (auto id : g_pointer_to_id.get(field)) {
                        if (g_id_to_object.count(id)) {
                            ...
                            *static_cast<Object**>(*field_it) = nullptr;
                            ...
                            error += fmt::format(
                                FMT_STRING(
                                    "Failed to find object with id {} for field {} of type {}\n"),
