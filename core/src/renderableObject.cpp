...
auto PTS::RenderableObject::from_obj(Scene const& scene, Material mat, std::string_view filename, std::string* warning) noexcept 
...
auto PTS::RenderableObject::make_triangle_obj(Scene const& scene, Material mat, Transform trans) noexcept -> RenderableObject {
	std::vector<Vertex> vertices = {
...
auto PTS::RenderableObject::make_quad_obj(Scene const& scene, Material mat, Transform trans) noexcept -> RenderableObject {
    std::vector<Vertex> vertices = {
        Vertex{ glm::vec3{ -0.5, -0.5, 0 }, glm::vec3{ 0, 0, 1 }, glm::vec2{ 0, 0 } },
        Vertex{ glm::vec3{ 0.5, -0.5, 0 }, glm::vec3{ 0, 0, 1 }, glm::vec2{ 1, 0 } },
...
auto PTS::RenderableObject::make_cube_obj(Scene const& scene, Material mat, Transform trans) noexcept -> RenderableObject {
    std::vector<Vertex> vertices = {
        // front
...
auto PTS::RenderableObject::make_sphere_obj(Scene const& scene, Material mat, Transform trans) noexcept -> RenderableObject {
...
auto PTS::RenderableObject::set_material(Material mat) noexcept -> void {
    m_mat = std::move(mat);
    if (m_mat.is_emissive()) {
        // if (!m_proxy_light) {
        //     m_proxy_light = Light{};
        // }
        // m_proxy_light->set_intensity(m_mat.emission_intensity);
        // m_proxy_light->set_color(m_mat.emission);
        // m_proxy_light->set_transform(get_transform());
    } else {
