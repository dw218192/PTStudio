...
auto PTS::SceneObject::set_transform(Transform transform, TransformSpace space) noexcept -> void {
    if (space == TransformSpace::LOCAL) {
        ...
        if (auto const par = get_parent()) {
            m_local_transform =
                par->get_transform(TransformSpace::WORLD).inverse() * m_world_transform;
        } else {
...
auto PTS::SceneObject::find_child(View<SceneObject> child) const
