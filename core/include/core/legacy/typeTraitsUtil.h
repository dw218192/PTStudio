...
template <typename T>
struct is_reflectable<T, std::void_t<decltype(std::declval<T>().get_class_name())>>
    : std::true_type {};
template <typename T>
...
template <typename T>
struct has_serialization_callback<T, std::void_t<decltype(std::declval<T>().on_serialize())>>
    : std::true_type {};
template <typename T>
constexpr bool has_serialization_callback_v = has_serialization_callback<T>::value;

template <typename T, typename = void>
struct has_deserialization_callback : std::false_type {};
template <typename T>
struct has_deserialization_callback<T, std::void_t<decltype(std::declval<T>().on_deserialize())>>
    : std::true_type {};
template <typename T>
constexpr bool has_deserialization_callback_v = has_deserialization_callback<T>::value;

template <typename T, typename = void>
struct has_dynamic_info : std::false_type {};
template <typename T>
struct has_dynamic_info<T, std::void_t<decltype(std::declval<T>().dyn_get_class_info())>>
    : std::true_type {};
template <typename T>
...
struct is_associative_container<
    T, std::void_t<decltype(std::declval<T>().find(std::declval<typename T::key_type>())),
...
struct is_sequence_container<
    T, std::void_t<decltype(std::declval<T>().size()), decltype(std::declval<T>().empty()),
                   decltype(std::declval<T>().rbegin()), decltype(std::declval<T>().rend()),
                   decltype(std::declval<T>().cbegin()), decltype(std::declval<T>().cend()),
                   decltype(std::declval<T>().crbegin()), decltype(std::declval<T>().crend()),
                   decltype(std::declval<T>().front()), decltype(std::declval<T>().back()),
...
template <typename L, typename R>
struct is_greater_than_comparable<L, R,
...
template <auto MemFunc, typename ClassType, typename... Args>
constexpr bool is_const_callable_method_v =
    is_const_callable_method<MemFunc, ClassType, Args...>::value;
...
struct for_each {
    ...
    static constexpr auto element_in_tuple(TupleLike&& tuple, Callable&& callable) {
        if constexpr (is_tuple_like_v<std::decay_t<TupleLike>>) {
            std::apply([&callable](
                           auto&&... args) { (callable(std::forward<decltype(args)>(args)), ...); },
    ...
    static constexpr auto type_in_tuple(Callable&& callable) {
        if constexpr (is_tuple_like_v<std::decay_t<TupleLike>>) {
            type_in_tuple_impl<TupleLike>(
                std::forward<Callable>(callable),
                std::make_index_sequence<std::tuple_size_v<std::decay_t<TupleLike>>>{});
        } else {
            static_assert(is_tuple_like_v<std::decay_t<TupleLike>>,
                          "for_each requires a tuple-like type");
        }
   ...
   private:
    ...
    static constexpr auto type_in_tuple_impl(Callable&& callable, std::index_sequence<Is...>) {
