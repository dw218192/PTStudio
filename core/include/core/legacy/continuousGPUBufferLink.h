...
struct ContinuousGPUBufferLink {
    ...
    using on_erase_callback_fn_t = void(UserDataType& user_data, cpu_handle_t handle,
                                        GPUType const& erased_val);
    using on_push_back_callback_fn_t = void(UserDataType& user_data, cpu_handle_t handle,
                                            GPUType const& new_val);
    using on_update_callback_fn_t = void(UserDataType& user_data, cpu_handle_t handle,
                                         GPUType const& new_val);
    ...
    struct Proxy {
        ...
        operator GPUType&() noexcept {
            return gpu_handle;
        }
        operator GPUType const&() const noexcept {
            return gpu_handle;
        }
    };
    ...
    NODISCARD auto at(size_t idx) const -> GPUType const& {
    ...
    NODISCARD auto get_on_erase_callbacks() noexcept -> auto& {
        return m_on_erase_callbacks;
    }
    NODISCARD auto get_on_push_back_callbacks() noexcept -> auto& {
        return m_on_push_back_callbacks;
    }
    NODISCARD auto get_on_update_callbacks() noexcept -> auto& {
        return m_on_update_callbacks;
    }
    NODISCARD auto get_user_data() noexcept -> auto& {
        return m_user_data;
    }
...
auto ContinuousGPUBufferLink<GPUType, CPUType, UserDataType, N>::erase(
    ...
    try {
        ...
        return TL_ERROR("Out of range, attempting to swap {} with {}: {}", idx, m_size - 1,
                        err.what());
    }
...
template <typename GPUType, typename CPUType, typename UserDataType, size_t N>
auto ContinuousGPUBufferLink<GPUType, CPUType, UserDataType, N>::get_ins(size_t idx) const noexcept
    -> tl::expected<cpu_handle_t, std::string> {
