...
struct SceneObject : Object {
   ...
   private:
    auto find_child(View<SceneObject> child) const
        -> std::vector<ObserverPtr<SceneObject>>::const_iterator;
    auto update_transform() noexcept -> void;

    BEGIN_REFLECT(SceneObject, Object);
    FIELD(Transform, m_world_transform, {}, MSerialize{}, MNoInspect{});  // handled explicitly
    FIELD(Transform, m_local_transform, {}, MSerialize{}, MNoInspect{});  // handled explicitly

    FIELD(EditFlags, m_flags, k_editable_flags, MSerialize{}, k_edit_flags_modifier);

    FIELD(ObserverPtr<Scene>, m_scene, nullptr, MSerialize{}, MReadOnly{});         // not editable
    FIELD(ObserverPtr<SceneObject>, m_parent, nullptr, MSerialize{}, MReadOnly{});  // not editable
    FIELD(std::vector<ObserverPtr<SceneObject>>, m_children, {}, MSerialize{},
          MReadOnly{});  // not editable
