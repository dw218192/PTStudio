...
from tasks.utils import ProjectContext, ShellCommand, is_windows, logger
...
def run(ctx: ProjectContext, args: dict[str, Any]) -> None:
    ...
    if conanrun_path.exists():
        ...
        if conan_deps_root.is_dir():
            ...
            extra_env = {"PATH": f"{conan_deps_root}{path_sep}{os.environ.get('PATH', '')}"}
        else:
            extra_env = None
...
@click.command(name="usdz", help="Package USDA scenes as USDZ archives via usdz_pack")
@project_options
@click.pass_context
