...
def _resolve_pts_shaderc(build_dir: Path) -> Path:
    ...
    it was staged there by a preceding `pixi run build --host-tools-only`
    ...
    raise FileNotFoundError(
        ...
        f"build dir; run `pixi run build --host-tools-only` first"
    )
...
def run(ctx: ProjectContext, args: dict[str, Any]) -> None:
    args = ctx.arguments("slangc", {"force": False}, args)
